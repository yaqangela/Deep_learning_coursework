...
class Attention:
    ...
    def forward(self, X):
        ...
        attn_mask = torch.triu(torch.ones(X.size(1), X.size(1)), diagonal=1)

        # Calculate/normalize Attention Scores
        masked = self.A_w.masked_fill(attn_mask.bool(), -torch.inf)
        self.d_out_kq = self.K.shape[2]
        self.A_sig = self.softmax.forward(masked / self.d_out_kq**0.5)
    ...
    def backward(self, dLdXnew):
        ...
        dLdA_w = self.softmax.backward(dLdA_sig) * 1 / self.d_out_kq**0.5
        ...
        self.dLdWq = torch.sum(torch.bmm(self.X.permute(0, 2, 1), self.dLdQ), dim=0)
        self.dLdWv = torch.sum(torch.bmm(self.X.permute(0, 2, 1), self.dLdV), dim=0)
        self.dLdWk = torch.sum(torch.bmm(self.X.permute(0, 2, 1), self.dLdK), dim=0)
